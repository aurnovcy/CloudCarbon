"""
Recommendations API router (synchronous).
"""
...
def list_recommendations(
    ...
    _sort_map = {"impact_score": "impact_score", "cost": "cost_impact_monthly_usd",
                 "co2e": "co2e_impact_monthly_kg", "water": "water_impact_monthly_litres"}
    order_col = "co2e_impact_monthly_kg" if (sort_by == "impact_score" and mode == "sustainability") else _sort_map.get(sort_by, "cost_impact_monthly_usd")
    ...
    list_sql = text(f"""
        ...
        ORDER BY {order_col} DESC LIMIT :limit OFFSET :offset
    """)
    ...
    items = [
        RecommendationResponse(
            ...
            cost_impact_monthly_usd=float(row[7] or 0), co2e_impact_monthly_kg=float(row[8] or 0),
            water_impact_monthly_litres=float(row[9] or 0), impact_score=float(row[10] or 0),
            cost_weight_used=float(row[11] or 0), carbon_weight_used=float(row[12] or 0),
            water_weight_used=float(row[13] or 0), complexity=row[14] or "low",
            implementation_steps=row[15] or [], methodology_notes=row[16] or "",
            policy_blocked=bool(row[17]), policy_block_reason=row[18],
            requires_approval=bool(row[19]), status=row[20] or "open",
            created_at=row[21], updated_at=row[22],
        )
        for row in rows
    ]
    return RecommendationListResponse(items=items, total=int(total), page=(offset // limit) + 1, page_size=limit)
...
def update_recommendation(
    ...
    check_sql = text("SELECT id FROM recommendations WHERE id = :id AND tenant_id = :tenant_id")
    row = db.execute(check_sql, {"id": str(recommendation_id), "tenant_id": str(tenant_id)}).fetchone()
    if not row:
        raise HTTPException(status_code=404, detail="Recommendation not found")

    update_sql = text("""
        UPDATE recommendations
        SET status = :status, snooze_until = :snooze_until, updated_at = :now
        ...
        RETURNING id, tenant_id, type, resource_id, provider, region, service_name,
                  ...
                  status, created_at, updated_at
    """)
    updated = db.execute(update_sql, {
        "status": body.status, "snooze_until": body.snooze_until, "now": now,
    ...
    db.execute(audit_sql, {
        "id": str(uuid.uuid4()), "tenant_id": str(tenant_id), "user_id": str(current_user.id),
        "resource_id": str(recommendation_id),
        "after_state": {"status": body.status, "snooze_until": str(body.snooze_until)}, "now": now,
    })
    db.commit()
...
def run_recommendations_endpoint(
    ...
    def _run():
        _db = SessionLocal()
        try:
            count = run_recommendations(tenant_id, weights, _db)
    ...
    return RecommendationRunResponse(job_id=job_id, tenant_id=tenant_id, status="queued", queued_at=now)
...
def recommendations_summary(
    ...
    summary = get_recommendations_summary(current_user.tenant_id, db)
